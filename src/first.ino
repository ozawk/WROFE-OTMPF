...
#define STEER_L_END 15             // POINT ステア左の終端角度 超過禁止
#define STEER_R_END 85             // POINT ステア右の終端角度 超過禁止
...
void setup() // 初回に1度だけ実行
{
    ...
    {                        // ボタンの長押し判定待ち受け
        ...
        { // 開始
            ...
            if (guess_start_pos_lr_ratio_ave < 0.35) // C周りと仮定する まだ車体の回転方向はわからないから
            {
                start_pos_i_c_o = 2; // in:0 center:1 out:2 C周りと仮定
                a = 0;
                buzz_three();
            }
            else if (guess_start_pos_lr_ratio_ave < 0.55) // どっちにしろC周り
            {
                start_pos_i_c_o = 1; // i:0 c:1 o:2 どっちにしろC周り
                a = 1;
                buzz_two();
            }
            else
            ...
            { // 走行開始
                ...
                Serial.print(" R:");
                Serial.println(r);
                if (is_start_turn_right_first(l, r) == 1) // どっち回りかわからないので両方調べる
                {
                    court_c_or_ccw = 0; // C周りと確定
                    Serial.println("=DBG= CONFIRM CLOCKWISE");
                    break;
                }
                else if (is_start_turn_left_first(l, r) == 1) // どっち回りかわからないので両方調べる
                {
...
void loop()
{
    ...
    if (flag_stby_push_move == 1)
    {
        ...
        if (now_what_num_area_cont >= 14) // POINT **どうせ1回ぐらい多く回っちゃうから賭けで一回長く走らせる? 13回以上曲がったら3週が終了と判定する
        {
            steer_ctrl(0, 0, 3);
...
int is_start_turn_right_first(int l, int r) // 初回R折
{
    int b = 0;
    if (a == 0) // i/o
    {
    ...
    if (r > b || r == 0) // POINT LRで特性が違うので注意 一定距離もしくは無限であれば 動かすなら最初に
    {
        start_turn_right_cont = start_turn_right_cont + 1;
        Serial.print("=DBG= TURN RIGHT FIRST REACH:");
    ...
    if (start_turn_right_cont > 3) // POINT これ変えるのもあり，でもなぜかちょっと増やすだけでレスポンス終わる謎 4回曲がる判定になれば曲がる
    {
        return 1;
    }
    else
...
int is_start_turn_left_first(int l, int r) // 初回L折
{
    int b = 0;
    if (a == 0) // i/o
    {
    ...
    if (l > b || l == 0) // POINT LRで特性が違うので注意 一定距離もしくは無限であれば 動かすなら最初に
    {
        start_turn_left_cont = start_turn_left_cont + 1;
        Serial.print("=DBG= TURN LEFT FIRST REACH:");
    ...
    if (start_turn_left_cont > 3) // POINT これ変えるのもあり，でもなぜかちょっと増やすだけでレスポンス終わる謎 4回曲がる判定になれば曲がる
    {
        return 1;
    }
    else
...
int is_start_turn_right(int l, int r) // 右に曲がるかどうかを判定 LR距離を入力する
{
    ...
    if (start_turn_right_cont > 8) // POINT これ変えるのもあり，でもなぜかちょっと増やすだけでレスポンス終わる謎 4回曲がる判定になれば曲がる
    {
        return 1;
    }
    else
...
{
    if (l > 55 || l == 0) // POINT LRで特性が違うので注意 一定距離もしくは無限であれば 動かすなら最初に
    {
        start_turn_left_cont = start_turn_left_cont + 1;
